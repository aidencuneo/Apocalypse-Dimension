...
public class mcreator_apocalypseDimension {
	...
	static {
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("apocalypseDimension_portal"));
		block = (ModTrigger) (new ModTrigger().setUnlocalizedName("apocalypseDimension_trigger"));
	}

	public mcreator_apocalypseDimension() {
	}

	public static DimensionType dtype = DimensionType.register("apocalypseDimension", "_apocalypseDimension", DIMID,
			mcreator_apocalypseDimension.WorldProviderMod.class, false);

	public void load(FMLInitializationEvent event) {

		portal.setRegistryName("testenvironmentmod_portal");
		GameRegistry.register(portal);
		GameRegistry.register(new ItemBlock(portal).setRegistryName(portal.getRegistryName()));

		block.setRegistryName("testenvironmentmod_trigger");
		GameRegistry.register(block);
		DimensionManager.registerDimension(DIMID, dtype);

		if (event.getSide() == Side.CLIENT)
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation("testenvironmentmod:testenvironmentmod_trigger", "inventory"));

		GameRegistry.addRecipe(
				new ItemStack(block, 1),
				new Object[]{"XX2", "X4X", "6XX", Character.valueOf('2'), Items.IRON_INGOT, Character.valueOf('4'), Items.IRON_INGOT,
						Character.valueOf('6'), Items.IRON_INGOT,});
					...
					for (int i1 = -2; i1 <= 2; ++i1) {
						for (int j1 = -1; j1 < 3; ++j1) {
							...
							this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2),
									flag ? Blocks.COBBLESTONE.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (j3 = 0; j3 < 4; ++j3) {
				for (k3 = 0; k3 < 4; ++k3) {
					for (l3 = -1; l3 < 4; ++l3) {
						...
						this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? Blocks.COBBLESTONE.getDefaultState() : iblockstate,
								2);
					}
				}
	...
	public static class BlockTutorialPortal extends Block {
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == Blocks.COBBLESTONE || getBlock(par1World, par2 + 1, par3, par4) == Blocks.COBBLESTONE) {
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == Blocks.COBBLESTONE || getBlock(par1World, par2, par3, par4 + 1) == Blocks.COBBLESTONE) {
				b1 = 1;
			}
			if (b0 == b1) {
				return false;
		...
		public void neighborChanged(IBlockState state, World par1World, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != Blocks.COBBLESTONE) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.COBBLESTONE) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.COBBLESTONE || getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.COBBLESTONE || getBlock(par1World, par2 + b0, par3,
										par4 + b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		...
		private IBlockState oceanBlock = mcreator_crackedClay.block.getDefaultState();
		...
		protected static final IBlockState GRAVEL = Blocks.SANDSTONE.getDefaultState();
			...
			for (int j1 = 255; j1 >= 0; --j1) {
				if (j1 <= rand.nextInt(5)) {
					...
					if (iblockstate2.getMaterial() == Material.AIR) {
						j = -1;
					} else if (iblockstate2.getBlock() == Blocks.SANDSTONE) {
						if (j == -1) {
							if (k <= 0) {
								iblockstate = AIR;
							...
							if (j1 < i && (iblockstate == null || iblockstate.getMaterial() == Material.AIR)) {
								iblockstate1 = mcreator_crackedClay.block.getDefaultState();
							}
		...
		public void populate(int x, int z) {
			...
			if (Biome != Biomes.DESERT && Biome != Biomes.DESERT_HILLS && this.settings.useWaterLakes && !flag
				...
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, flag,
					...
					(new WorldGenLakes(mcreator_crackedClay.block)).generate(this.worldObj, this.rand, blockpos.add(i1, j1, k1));
				}
			...
			if (!flag && this.rand.nextInt(this.settings.lavaLakeChance / 10) == 0 && this.settings.useLavaLakes)
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, flag,
					...
					if (l2 < this.worldObj.getSeaLevel() || this.rand.nextInt(this.settings.lavaLakeChance / 8) == 0) {
						(new WorldGenLakes(mcreator_crackedClay.block)).generate(this.worldObj, this.rand, blockpos.add(i2, l2, k3));
					}
				}
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		...
		protected Biome[] allowedBiomes = {Biome.REGISTRY.getObject(new ResourceLocation("desert")),};
